...
@router.post("/email-fixtures/")
async def extract_email_fixtures(payload: EmailProcessingRequest):
    """Generate raw/net fixtures from recent job alert emails."""
    email_address = os.getenv("EMAIL_ADDRESS")
    password = os.getenv("EMAIL_PASSWORD")
    if not email_address or not password:
        raise RuntimeError("email_address or password is not set.")
    ...
    generated = service.extract_tests_fixtures(days_back=payload.days_back)
