...
class IndeedParser(EmailParser):
    ...
    def matches(self, sender: str, subject: str) -> bool:
        ...
        return "indeed" in sender.lower() and (
            "alert" in subject.lower() or "offre" in subject.lower()
        )
    ...
    def parse(self, html: str) -> list[dict]:
        ...
        offer_rows = soup.find_all(
            "td", style=lambda s: s is not None and "padding:0px 0px 32px" in s
        )
        ...
        for row in offer_rows:
            ...
            # ----- (2) Extract job title -----
            title_link = row.find(
                "a", style=lambda s: s is not None and "font-size:16px" in s
            )
            if not title_link:
                continue
