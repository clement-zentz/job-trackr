...
class JobApplicationRepository:
    ...
    async def get_by_id_with_job_opportunity(
        self,
        ...
        stmt = (
            ...
            .options(selectinload(JobApplication.job_opportunity))
        )
        result = await self.session.execute(stmt)
        return result.scalar_one_or_none()

    async def list_with_job_opportunity(
        self,
    ) -> list[JobApplication]:
        stmt = select(JobApplication).options(
            selectinload(JobApplication.job_opportunity)
        )
        result = await self.session.execute(stmt)
